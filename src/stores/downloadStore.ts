...
/** 行上最终展示的状态。 */
export type DownloadState = DownloadStatus | "inLibrary" | "idle";
...
export const useDownloadStore = defineStore("download", () => {
  ...
  /**
   * 行上要展示的状态。优先级：下载中 > 本次已下载 > 曲库已有 > 失败 > 空闲。
   *
   * inLibrary 排在 failed 之前：文件就在曲库里却显示「重试」是错的，那个
   * 操作也没有意义。done 排在 inLibrary 之前同样不能反：反过来下载成功会
   * 立刻自降级成「已在曲库」，把用户要的那个打勾确认弄丢。
   *
