export default {
  common: {
    localMusic: "曲库",
    onlineSearch: "在线音乐",
    settings: "设置",
    unknownArtist: "未知歌手",
    unknownSong: "未知歌曲",
    back: "返回",
    browse: "浏览",
    reset: "重置",
    enable: "启用",
    disable: "禁用",
    dark: "深色",
    light: "浅色",
    warm: "暖色",
    cancel: "取消",
    confirmDelete: "确定删除",
    close: "关闭",
    download: "下载",
  },
  playlist: {
    title: "播放列表",
    newPlaylist: "新建播放列表",
    unnamed: "未命名列表",
    rename: "重命名",
    delete: "删除",
    deleteConfirm: "确定要删除该播放列表吗？",
    empty: "列表为空，从本地或在线搜索添加歌曲",
    notFound: "播放列表不存在",
    addToPlaylist: "添加到播放列表",
    added: "已添加到「{name}」",
    alreadyInPlaylist: "该歌曲已在「{name}」中",
    downloadedAndAdded: "已下载并已添加到「{name}」",
    removeSelected: "从播放列表移除选中",
    playAll: "播放全部",
    trackCount: "{count} 首歌曲",
    browseLibrary: "前往曲库",
    browseOnline: "在线搜索",
  },
  header: {
    switchToLight: "切换到亮色模式",
    switchToDark: "切换到暗色模式",
    minimize: "最小化",
    restore: "还原",
    maximize: "最大化",
    close: "关闭",
  },
  search: {
    placeholderLocal: "搜索本地歌曲...",
    placeholderOnline: "搜索在线歌曲...",
    placeholderOnlineArtist: "搜索歌手...",
    placeholderOnlineAlbum: "搜索专辑...",
    placeholderOnlinePlaylist: "搜索歌单...",
    placeholderPlaylist: "搜索当前播放列表...",
    historyTitle: "搜索历史",
    clearHistory: "清除历史",
    removeItem: "删除该条",
    noHistory: "暂无搜索历史",
  },
  settings: {
    title: "设置",
    languageZh: "简体中文",
    languageEn: "English",
    appearance: "外观设置",
    themeMode: "主题模式",
    themeModeDesc: "选择应用的整体色彩风格",
    appSettings: "应用设置",
    autoStart: "开机自启动",
    autoStartDesc: "登录系统后自动运行 RMusic",
    download: "下载",
    downloadLocation: "路径",
    downloadLocationDesc: "本地曲库和下载文件的保存位置",
    selectDownloadLocation: "选择文件夹",
    libraryRoot: "根目录",
    cache: "缓存",
    onlineAudioCache: "在线音频缓存",
    onlineAudioCacheDesc: "清理不会删除本地曲库歌曲",
    cachePath: "缓存路径",
    clearCache: "清理缓存",
    about: "关于",
    aboutDesc: "一个基于 Tauri 和 Vue 的音乐播放器",
    language: "语言",
    languageDesc: "切换界面显示语言",
    onlineService: "在线服务",
    serviceStatus: "服务状态",
    serviceStatusDesc: "在线搜索和播放所需的后台服务",
    refreshService: "启动或刷新在线服务",
  },
  musicList: {
    title: "曲库",
    import: "导入",
    importFolder: "从文件夹导入音乐",
    empty: "暂无音乐，点击「导入音乐」添加",
    multiSelect: "多选",
    selectAll: "全选",
    deselectAll: "取消全选",
    cancelSelect: "取消多选",
    addSelectedToPlaylist: "添加到播放列表",
    selectedCount: "已选 {count} 首",
    updating: "正在后台更新曲库…",
    summary: "{count} 首歌曲 · {minutes} 分钟",
    columnSong: "歌曲",
    columnAlbum: "专辑",
    columnDuration: "时长",
  },
  onlineMusic: {
    title: "搜索",
    empty: "搜索歌曲开始播放",
    loadMore: "加载更多",
    artists: "歌手",
    resultSummary: "{count} 个结果",
    tabSong: "单曲",
    tabArtist: "歌手",
    tabAlbum: "专辑",
    tabPlaylist: "歌单",
    summarySong: "{count} 首单曲",
    summaryArtist: "{count} 位歌手",
    summaryAlbum: "{count} 张专辑",
    summaryPlaylist: "{count} 个歌单",
    refineHint: "结果过多，请细化搜索关键词",
  },
  onlineService: {
    checking: "正在检测在线服务",
    restarting: "正在刷新在线服务",
    available: "在线服务可用",
    unavailable: "在线服务不可用",
    clickToRestart: "点击刷新服务",
  },
  artist: {
    backToSearch: "返回搜索",
    open: "查看歌手：{name}",
    albums: "专辑",
    description: "歌手简介",
    hotSongs: "热门歌曲",
    allSongs: "全部歌曲",
    albumCount: "{count} 张专辑",
    songCount: "{count} 首歌曲",
  },
  onlinePlaylist: {
    title: "歌单",
    empty: "未找到相关歌单",
    back: "返回搜索",
    trackCount: "{count} 首",
    playCount: "{count} 次播放",
    creator: "创建者",
    notFound: "歌单不存在或已被删除",
  },
  onlineAlbum: {
    title: "专辑",
    empty: "未找到相关专辑",
    back: "返回搜索",
    songCount: "{count} 首",
    notFound: "专辑不存在或已被删除",
  },
  toplist: {
    title: "排行榜",
    empty: "排行榜暂时不可用",
    refresh: "刷新",
  },
  playerBar: {
    noSong: "未选择歌曲",
    emptyHint: "从曲库选择一首歌曲开始播放",
    previous: "上一曲",
    next: "下一曲",
    play: "播放",
    pause: "暂停",
    resolving: "正在获取播放地址…",
    buffering: "正在缓冲…",
    queue: "播放队列",
    currentQueue: "当前队列",
    queueEmpty: "队列中还没有歌曲",
    mute: "静音",
    unmute: "恢复音量",
    toggleRemainingTime: "切换总时长与剩余时间",
    sequential: "顺序播放",
    random: "随机播放",
    repeatOne: "单曲循环",
  },
  lyric: {
    noLyric: "暂无歌词",
    loadFailed: "歌词加载失败",
    loading: "加载歌词中...",
  },
  import: {
    audioFiles: "音频文件",
    importing: "正在导入 {count} 个文件...",
    failed: "导入失败",
    openDialogFailed: "打开文件选择对话框失败",
  },
  download: {
    starting: "开始下载歌曲，请稍候...",
    done: "歌曲已下载: {fileName}",
  },
  messages: {
    playing: "正在播放: {name}",
    noLocalMusic: "没有可播放的本地音乐",
    noOnlineMusic: "没有可播放的在线音乐",
    noSearchResult: "未找到相关歌曲",
    foundSongs: "找到 {count} 首相关歌曲",
    setDirSuccess: "默认下载目录设置成功",
    resetDirSuccess: "已重置为默认下载目录",
    playModeSwitch: "已切换到{mode}模式",
    cacheCleared: "缓存已清理",
    pathCopied: "路径已复制",
  },
  errors: {
    loadMusicFailed: "加载音乐文件失败",
    playFailed: "播放音乐失败",
    playFailedOnline: "播放失败",
    downloadFailed: "下载歌曲失败",
    fileAlreadyExists: "文件已存在，无需重复下载",
    fileAlreadyExistsWithPath: "文件已存在: {fileName}",
    switchFailed: "切换歌曲失败",
    togglePlayFailed: "切换播放状态失败",
    searchFailed: "在线搜索失败",
    loadPlaylistFailed: "加载歌单失败",
    loadAlbumFailed: "加载专辑失败",
    loadToplistFailed: "加载排行榜失败",
    loadArtistAlbumsFailed: "加载歌手专辑失败",
    setDirFailed: "设置默认目录失败",
    resetDirFailed: "重置默认目录失败",
    clearCacheFailed: "清理缓存失败",
    networkError: "网络错误，请检查网络连接",
    apiError: "API 返回错误",
    fileSystemError: "文件系统错误",
    unknownError: "未知错误",
  },
} as const;
